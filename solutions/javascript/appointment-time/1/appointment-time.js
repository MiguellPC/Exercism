...
export function updateAppointment(timestamp, options) {
  ...
  const setOpt = {
    year: (opt) => date.setFullYear(opt),
    month: (opt) => date.setMonth(opt),
    date: (opt) => date.setDate(opt),
    hour: (opt) => date.setHours(opt),
    minute: (opt) => date.setMinutes(opt)    
  }

  for (const val in options) {
    const opt = setOpt[val];
    opt(options[val])
  }
  
  const newDate = getAppointmentTimestamp(date);
